...
export function Menu({ node, deselect }) {
  ...
  function artificialCharacter (charTone) {
    ...
    axios.post("https://guai-server.onrender.com:10000/buttons", {charTone})
    .then((res) => {
  ...
  function artificial (prompt, brief, nodes, charTone) {
    ...
    if (prompt == "merge") {
      ...
      axios.post("https://guai-server.onrender.com:10000/buttons", {nodeLabel, prompt, brief, nodes, charTone})
      .then((res) => {
      ...
      axios.post("https://guai-server.onrender.com:10000/buttons", {prompt, brief, nodes, charTone}) // the var names here matter! nodeLabel and prompt are referred to in index., charTonejs
      .then((res) => {
      ...
      for (let n of selectedNodes) {
        ...
        axios.post("https://guai-server.onrender.com:10000/buttons", {nodeLabel, sourceLabels, targetLabels, prompt, brief, nodes, charTone})
        .then((res) => {
            setLabel(res.data);
        ...
        if (n.data.target) {for (let targetId of n.data.target) {
          ...
          axios.post("https://guai-server.onrender.com:10000/buttons", {nodeLabel, prompt, brief, sourceLabels, charTone}) // the var names here matter! nodeLabel and prompt are referred to in index., charTonejs
          .then((res) => {
      ...
      <h1> Menu </h1>
        ...
        <div className="personasContainer">
          <div className="persona">
              <img title="adopt a neutral, non-autonomous persona!" onClick={e => artificialCharacter(e.target.id)} id='off' style={(charTone=='off')? {'filter': 'none'} : {'filter': 'grayscale()'}} alt="hades" className="personaImgs" src="/public/Images/static/hades.png"></img>
              <div>
                  <p>Hades</p>
          ...
          <div className="persona">
              <img title="adopt a fully autonomous and creative persona, he interacts as he likes!" onClick={e => artificialCharacter(e.target.id)} id='abstract' style={(charTone=='abstract')? {'filter': 'none'} : {'filter': 'grayscale()'}} alt="Apollo" className="personaImgs" src="/public/Images/static/apollo.png"></img>
              <div>
                  <p>Apollo</p>
          ...
          <div className="persona">
              <img title="adopt an autonomous and succinct persona, customize how often she interacts!" onClick={e => artificialCharacter(e.target.id)} id='realistic' style={(charTone=='realistic')? {'filter': 'none'} : {'filter': 'grayscale()'}} alt="themis" className="personaImgs" src="/public/Images/static/themis.png"></img>
              <div>
                  <p>Themis</p>
