...
export function Menu({ node, deselect }) {
  ...
  function artificial (node, prompt, brief, nodes) {
    ...
    axios.post("http://https://guai-server.onrender.com/:10000/buttons", {nodelabel, prompt, brief, nodes}) // the var names here matter! nodelabel and prompt are referred to in index.js
    .then((res) => {
        if (prompt == "group") {
